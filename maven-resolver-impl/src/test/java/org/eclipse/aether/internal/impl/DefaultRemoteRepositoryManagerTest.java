...
public class DefaultRemoteRepositoryManagerTest {
    ...
    private RemoteRepository.Builder newRepo( String id, String url, boolean enabled, String updates,
                                              String checksums )
    {
    ...
    public void testGetPolicy() {
        ...
        RepositoryPolicy releasePolicy = new RepositoryPolicy( true, RepositoryPolicy.UPDATE_POLICY_NEVER,
    ...
    public void testAggregateSimpleRepos() {
        ...
        List<RemoteRepository> result = manager.aggregateRepositories( session, Arrays.asList( dominant1 ),
    ...
    public void testAggregateSimpleRepos_MustKeepDisabledRecessiveRepo() {
        ...
        List<RemoteRepository> result = manager.aggregateRepositories( session, Arrays.asList( dominant ),
    ...
    public void testAggregateMirrorRepos_DominantMirrorComplete() {
        ...
        RemoteRepository dominantMirror1 = newRepo( "x", "file://", false, "", "" ).addMirroredRepository( dominant1 )
                .build();

        RemoteRepository recessive1 = newRepo( "a", "https://", true, "", "" ).build();
        RemoteRepository recessiveMirror1 = newRepo( "x", "http://", true, "", "" ).addMirroredRepository( recessive1 )
                .build();

        List<RemoteRepository> result = manager.aggregateRepositories( session, Arrays.asList( dominantMirror1 ),
    ...
    public void testAggregateMirrorRepos_DominantMirrorIncomplete() {
        ...
        RemoteRepository recessiveMirror1 = newRepo( "x", "http://", true, "", "" )
                .setMirroredRepositories( Arrays.asList( recessive1, recessive2 ) ).build();
    ...
    public void testMirrorAuthentication() {
        ...
        final RemoteRepository mirror = newRepo( "a", "http://", true, "", "" )
                .setAuthentication( new AuthenticationBuilder().addUsername( "test" ).build() ).build();
        ...
        List<RemoteRepository> result = manager.aggregateRepositories( session,
                Collections.<RemoteRepository>emptyList(), Arrays.asList( repo ), true );
    ...
    public void testMirrorProxy() {
        final RemoteRepository repo = newRepo( "a", "http://", true, "", "" ).build();
        final RemoteRepository mirror = newRepo( "a", "http://", true, "", "" )
                .setProxy( new Proxy( "http", "host", 2011, null ) ).build();
        session.setMirrorSelector( new MirrorSelector() {
            public RemoteRepository getMirror( RemoteRepository repository ) {
                return mirror;
            }
