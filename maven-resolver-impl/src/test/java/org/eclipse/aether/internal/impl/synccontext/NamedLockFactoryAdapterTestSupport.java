...
public abstract class NamedLockFactoryAdapterTestSupport {
    ...
    public void justAcquire() {
        try( SyncContext syncContext = adapter.newInstance( session, false ) ) {
            syncContext.acquire( Arrays.asList( new DefaultArtifact( "groupId:artifactId:1.0" ),
                    new DefaultArtifact( "groupId:artifactId:1.1" ) ), null );
        }
    }
    ...
    @Test( timeout = 5000 )
    public void nestedSharedShared()
            throws InterruptedException
    {
        CountDownLatch winners = new CountDownLatch( 2 ); // we expect 2 winners
        CountDownLatch losers = new CountDownLatch( 0 ); // we expect 0 losers
        Thread t1 = new Thread( new Access( true, winners, losers, adapter, session,
    ...
    @Test( timeout = 5000 )
    public void nestedExclusiveShared()
            throws InterruptedException
    {
        CountDownLatch winners = new CountDownLatch( 2 ); // we expect 2 winners
        CountDownLatch losers = new CountDownLatch( 0 ); // we expect 0 losers
        Thread t1 = new Thread( new Access( false, winners, losers, adapter, session,
                                            new Access( true, winners, losers, adapter, session, null ) ) );
        t1.start();
        t1.join();
        winners.await();
    ...
    @Test( timeout = 5000 )
    public void nestedExclusiveExclusive()
            throws InterruptedException
    {
        CountDownLatch winners = new CountDownLatch( 2 ); // we expect 2 winners
        CountDownLatch losers = new CountDownLatch( 0 ); // we expect 0 losers
    ...
    @Test( timeout = 5000 )
    public void nestedSharedExclusive()
            throws InterruptedException
    {
    ...
    @Test
    public void fullyConsumeLockTime()
            throws InterruptedException
    {
    ...
    @Test
    public void releasedExclusiveAllowAccess()
            throws InterruptedException
    {
            ...
            implements Runnable
    {
        ...
        public Access( boolean shared, CountDownLatch winner, CountDownLatch loser, NamedLockFactoryAdapter adapter,
                       RepositorySystemSession session, Access chained )
        {
            this.shared = shared;
            this.winner = winner;
            this.loser = loser;
