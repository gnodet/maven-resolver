        ...
        implements ChecksumAlgorithmFactorySelector
{
    ...
    public ChecksumAlgorithmFactory select( String algorithmName ) {
        ...
        if( factory == null ) {
            List<String> algorithms = getChecksumAlgorithmFactories().stream().map( ChecksumAlgorithmFactory::getName )
                    .collect( toList() );
            throw new IllegalArgumentException( String
                    .format( "Unsupported checksum algorithm %s, supported ones are %s", algorithmName, algorithms ) );
        }
        return factory;
    }
