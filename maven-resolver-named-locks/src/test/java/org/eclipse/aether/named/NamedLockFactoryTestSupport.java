...
public abstract class NamedLockFactoryTestSupport {
    ...
    public void refCounting() {
        ...
        try( NamedLock one = namedLockFactory.getLock( name ); NamedLock two = namedLockFactory.getLock( name ) ) {
            ...
            implements Runnable
    {
        ...
        public Access( NamedLockFactory namedLockFactory, String name, boolean shared, CountDownLatch winner,
        ...
        public void run() {
            try( NamedLock lock = namedLockFactory.getLock( name ) ) {
                if( shared ? lock.lockShared( ACCESS_WAIT_MILLIS, TimeUnit.MILLISECONDS )
